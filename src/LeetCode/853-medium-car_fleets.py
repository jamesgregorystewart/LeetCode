...
"""
Idea:
    - nlogn sort the arrays in order of position and evaluate left to right
"""
...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # each car has a position and speed: car[0] and car[1] respectively
        for car in combined:
            hours = (target - car[0]) / car[1]
            while fleets and fleets[-1] <= hours:
                fleets.pop()
            fleets.append(hours)
        return len(fleets)
